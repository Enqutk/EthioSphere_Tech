      ...
      <div className="card p-8">
        <div className="flex flex-col items-start gap-6 sm:flex-row sm:items-center">
          ...
          <div className="min-w-0 flex-1">
            ...
            {profile.githubUrl && (
              <a href={profile.githubUrl} target="_blank" rel="noopener noreferrer" className="mt-2 inline-block text-brand-400 hover:underline">
                GitHub →
              </a>
            )}
          </div>
          ...
          <div className="mt-6 border-t border-slate-700 pt-6">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {profile.skills.map((s) => (
                <span key={s} className="rounded-md bg-surface-800 px-2 py-1 text-sm text-slate-300">
                  {s}
                </span>
              ))}
            </div>
          ...
          <div className="mt-6 border-t border-slate-700 pt-6">
            ...
            <ul className="mt-2 space-y-2">
              {profile.projectsOwned.map((p) => (
                <li key={p.id}>
                  <Link href={`/projects/${p.id}`} className="text-brand-400 hover:underline">
                    {p.title}
                  </Link>
          ...
          <div className="mt-6 border-t border-slate-700 pt-6">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {profile.badges.map((b) => (
                <span key={b.badgeType} className="rounded-md bg-amber-500/20 px-2 py-1 text-sm text-amber-400">
                  {b.badgeType}
                </span>
              ))}
            </div>
