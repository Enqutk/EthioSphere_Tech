    ...
    <div className="mx-auto max-w-4xl px-6 py-12">
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        <button
          type="button"
          onClick={() => setSection('')}
          className={`rounded-lg px-3 py-1.5 text-sm ${!section ? 'bg-brand-500/20 text-brand-400' : 'bg-surface-800 text-slate-400 hover:text-slate-200'}`}
        >
          All
        </button>
        {Object.entries(SECTIONS).map(([value, label]) => (
          <button
            ...
            className={`rounded-lg px-3 py-1.5 text-sm ${section === value ? 'bg-brand-500/20 text-brand-400' : 'bg-surface-800 text-slate-400 hover:text-slate-200'}`}
          >
            {label}
          </button>
        ))}
      ...
      {loading ? (
        ...
        <div className="mt-12 rounded-xl border border-dashed border-slate-700 p-12 text-center text-slate-500">
        ...
        <ul className="mt-8 space-y-4">
          {posts.map((p) => (
            <li key={p.id}>
              <Link href={`/community/${p.id}`} className="card block p-6 transition hover:border-brand-500/50">
