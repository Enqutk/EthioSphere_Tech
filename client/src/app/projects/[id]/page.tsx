...
export default function ProjectDetailPage() {
  ...
  const isMember = project.owner.id === user?.id || project.members.some((m) => m.user.id === user?.id);
  const canJoin = user && !isMember && project.status !== 'ARCHIVED';

  return (
    <div className="mx-auto max-w-3xl px-6 py-12">
      <Link href="/projects" className="text-sm text-slate-400 hover:text-brand-400">← Back to projects</Link>
      <div className="card mt-4 p-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <Link href={`/profile/${project.owner.username}`} className="flex items-center gap-2 text-sm text-slate-400 hover:text-brand-400">
            {project.owner.avatarUrl ? (
              <img src={project.owner.avatarUrl} alt="" className="h-8 w-8 rounded-full" />
            ) : (
              <span className="flex h-8 w-8 items-center justify-center rounded-full bg-surface-800 text-slate-500">{project.owner.name.charAt(0)}</span>
            )}
          ...
          <div className="mt-8 border-t border-slate-700 pt-6">
            ...
            <ul className="mt-2 space-y-2">
              <li className="flex items-center gap-2 text-sm">
                ...
                <Link href={`/profile/${project.owner.username}`} className="text-slate-300 hover:text-brand-400">
              ...
              {project.members.map((m) => (
                <li key={m.user.id} className="flex items-center gap-2 text-sm">
                  ...
                  <Link href={`/profile/${m.user.username}`} className="text-slate-300 hover:text-brand-400">
                    {m.user.username}
                  </Link>
                </li>
              ))}
